...
public class OperationAdapterTest extends AsfsAdapterBaseTest {
  ...
  public void testRequestCommitOperation() {
    final SrvCommitOperationRq request = OperationAdapter.requestCommitOperation(OPERATION_ID);
  ...
  public void testRequestCreateOperation() {
    ...
    final SrvCreateOperationRq request = OperationAdapter.requestCreateOperation(operation);
    assertHeaderInfo(request.getHeaderInfo());
  ...
  public void testRequestRollbackOperation() {
    ...
    final SrvRollbackOperationRq request =
        OperationAdapter.requestRollbackOperation(operation);
    assertHeaderInfo(request.getHeaderInfo());
  ...
  public void testRequestUpdOperation() {
    final SrvUpdOperationRq request =
        OperationAdapter.requestUpdOperation(createOperation());
    assertHeaderInfo(request.getHeaderInfo());
  ...
  public void testConvertSrvGetOperationRs() {
    final ExternalEntityList<Operation> list = OperationAdapter.convert(responseGetOp);
  ...
  public void testMultiAdapter() {
    ...
    Assert.assertNotNull(getOpExternalEntity);
    Assert.assertEquals(ExternalEntityList.class, getOpExternalEntity.getClass());
    Assert.assertNotNull(((ExternalEntityList) getOpExternalEntity).getItems());
    Assert.assertEquals(OPERATION_ITEM_LIST_SIZE,
        ((ExternalEntityList) getOpExternalEntity).getItems().size());
    Assert.assertEquals(Operation.class,
        ((ExternalEntityList) getOpExternalEntity).getItems().get(0).getClass());
  }
}